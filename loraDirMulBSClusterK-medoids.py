...
class myBS():
    def __init__(self, id):
        ...
        self.x = posxBS[self.id]
        self.y = posyBS[self.id]
...
class myNode():
    ...
    def createPacket(self, i, packetlen):
        ...
        for i in range(0,nrBS):
            ...
            print (self.x, self.y, bs[i].x, bs[i].y)
            self.dist.append(d)
            self.packet.append(myPacket(self.id, packetlen, self.dist[i], i))
            print "distance to base station: " + str(self.dist[i])
            print "maxDist: " + str(maxDist)
        print('node %d' %self.id, "x", self.x, "y", self.y, "dist: ", self.dist)
...
if len(sys.argv) >= 5:
    ...
    posyBS[1] = int(sys.argv[9])
    posxBS[2] = int(sys.argv[10])
    posyBS[2] = int(sys.argv[11])
    """
    
    data = np.loadtxt("nodes_placed.dat")
    nrNodes = len(data)
...
Lpl = Ptx - minsensi
print "lpl2: " + str(Lpl)
#print "amin", minsensi, "Lpl", Lpl  minsensi est appelé amin dans le code... va savoir pk
print "minsensi", minsensi, "Lpl", Lpl
maxDist = d0*(math.e**(math.log(10)*(Lpl-Lpld0)/(10.0*gamma)))
...
#On crée les paquets virtuels maintenant que les bs sont placées.
for i in range(0, len(nodes)):
    # myNode takes period (in ms), base station id packetlen (in Bytes)
    # 1000000 = 16 min
    nodes[i].createPacket(i, 20)
    env.process(transmit(env,nodes[i]))

##

#prepare show
#Redimensionnement de la fenêtre de vue
if (graphics == 1):
    plt.xlim([-50, 1500])
    plt.ylim([-50, 900])
    plt.draw()
...
fname = "exp" + str(experiment) + "BSClusterK-medoids" + str(nrBS) + ".dat"
print fname
